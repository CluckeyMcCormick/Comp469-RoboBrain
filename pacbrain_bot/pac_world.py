...
class Pacworld:
	...
	def reset(self, useRandom=False):
		...
		#                 y, x
		self.pac_pos   = ( int(self.ROWS / 2), int(self.COLS / 2) )#(6, 1)
		self.red_pos   = (random.randint(1, self.ROWS - 2), self.COLS - 2) #(6, 5)
		self.green_pos = (random.randint(1, self.ROWS - 2), 1) #(1, 1)

		self.red_dir =   (random.randint(0, 1) * 2) + 1
		self.green_dir = (random.randint(0, 1) * 2) + 1
